...
```{r}
#import dataset using readr's read_csv
transactions <- read_csv("transactions.csv")
```

## 1. a. Cleaning the data

Readr did not identify the date column as the specific data type of date, so we use the parse_date functionto parse the values in the date colum as dates.
...
```{r}
#Use the date parser to get the data data into an understandable format which matches the data dictionary
transactions$date <- parse_date(transactions$date, format = "%d%m%y")
#Write a new 'clean' csv with correct date formates and remove and artefacts from opening in Excel
...
Let's visualise some of the variables to see what 
...
```
## 2. a. ii. Create a line plot of the variable monthly_amoount for industry = 1 and location = 1
