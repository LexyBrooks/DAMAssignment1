...
calculate_predictions <- function(df, industries, locations) {
  ...
  for (ind in industries) {
    for (loc in locations) {
      ...
      if (length(unique(temp$date)) >= ceiling(trainingMultiplier*testingNum)) {
  ...
  colnames(output) <- c("industry","location", "train_rmse", "Train Dec 2016 Prediction")
...
calculate_predictions(aggregated_transactions, 1, 1)
...
# Order by RMSE
arrange(all_predictions, train_rmse)
