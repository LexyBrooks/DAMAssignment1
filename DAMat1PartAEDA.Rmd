...
Create line plots of the variable monthly_amount for industry = 2 and location = 2
...
Create a line plot of the variable monthly_amount for industry = 5 and location = 5
...
Create a line plot of the variable monthly_amount for location = 6
...
Create a line plot of the variable monthly_amount for industry = 6
...
Create a line plot of the variable monthly_amount for industry = 7 and location = 7
...
Create a line plot of the variable monthly_amount for industry = 8 and location = 8
...
Create a line plot of the variable monthly_amount for industry = 9 and location = 9
...
Create a line plot of the variable monthly_amount for industry = 10 and location = 10
...
```{r}
ggplot(data=filter(aggregated_transactions, industry == 10), aes(x=date, y=monthly_amount, group=1)) + geom_line(color="red") + geom_point() + labs(title="Line plot of mean monthly amount for Industry 10 ONLY", x="Date", y="Mean Monthly Amount")
```
```{r}
ggplot(data=filter(aggregated_transactions, location == 10), aes(x=date, y=monthly_amount, group=1)) + geom_line(color="red") + geom_point() + labs(title="Line plot of mean monthly amount for location 10 ONLY", x="Date", y="Mean Monthly Amount")
```
### 2. a. iii) i) Train a linear regression model with monthly_amount as the target for location = 1 and industry = 1

TODO: We want to explore different models, with monthly_amount always the target, but trialling different predictors.
...
# This value is explained in the function below, but was tuned based on analysis
...
#accuracy diagnotics
length(coefficients(month_number_and_time_number.lm))
MSE <- month_number_and_time_number.lm/length(month_number_and_time_number.lm$residuals)
RMSE <- sqrt(MSE)
...
```{r}
# Train a linear regression model and return predictions including the RMSE
make_predictions <- function(df, testingNum, ind, loc, setToTest = "full", forecast = FALSE) {
...
# Calculate the predictions for all industry and location pairs on test data
prediction_for_all_industry_location <- return_predictions(aggregated_transactions, industries, locations, setToTest = "test", forecast = TRUE)
